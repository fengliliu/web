...
if BigClassName="" then
	founderr=true
	errmsg=errmsg+"<li>未指定信息所属大类</li>"
end if
if Title="" then
	founderr=true
	errmsg="<li>信息标题不能为空</li>"
end if
if Content="" then
	founderr=true
	 ...
	 case "Modify"	
  		if ID<>"" then
			...
			if not (rs.bof and rs.eof) then
				...
				founderr=true
				errmsg=errmsg+"<li>找不到此信息，可能已经被删除。</li>"
				call WriteErrMsg()
			end if
		else
			founderr=true
	...
	if ObjInstalled=True and UploadFiles<>"" then
		...
		if instr(UploadFiles,"|")>1 then
			...
			for intTemp=0 to ubound(arrUploadFiles)
				if instr(Content,arrUploadFiles(intTemp))<=0 and arrUploadFiles(intTemp)<>FirstImageName then
					...
					if fso.FileExists(strRubbishFile) then
						...
						response.write "<br><li>" & arrUploadFiles(intTemp) & "在信息中没有用到，也没有被设为首页图片，所以已经被删除！</li>"
					end if
				else
					if intTemp=0 then
			...
			if instr(Content,UploadFiles)<=0 and UploadFiles<>FirstImageName then
				...
				if fso.FileExists(strRubbishFile) then
					fso.DeleteFile(strRubbishFile)
					response.write "<br><li>" & UploadFiles & "在信息中没有用到，也没有被设为首页图片，所以已经被删除！</li>"
				end if
				UploadFiles=""
			end if
		end if
