...
<form name="form1" method="post" action="Admin_Class_Menu.asp" onSubmit="return check()">
  <table width="100%" border="0" align="center" cellpadding="2" cellspacing="1" class="table_southidc">
    ...
    <tr class="td_southidc"> 
      <td><strong>是否在内容导航栏显示：</strong><br>
        此选项只对一级菜单有效。</td>
      <td><input name="ShowOnLeft" type="radio" value="Yes" >
        是&nbsp;&nbsp;&nbsp;&nbsp; <input type="radio" name="ShowOnLeft" value="No" checked>
        否</td>
    </tr>
    <tr class="td_southidc"> 
...
<form name="form1" method="post" action="Admin_Class_Menu.asp" onSubmit="return check()">
  <table width="100%" border="0" align="center" cellpadding="2" cellspacing="1" class="table_southidc">
    ...
    <tr class="td_southidc"> 
      <td><strong>是否在内容导航栏显示：</strong><br>
        只选项只对一级菜单有效。</td>
      <td><input name="ShowOnLeft" type="radio" value="Yes" <%if rsClass("ShowOnLeft")=True then response.write " checked"%>>
        是&nbsp;&nbsp;&nbsp;&nbsp; <input type="radio" name="ShowOnLeft" value="No" <%if rsClass("ShowOnLeft")=False then response.write " checked"%>>
        否 </td>
    </tr>
    <tr class="td_southidc"> 
      <td width="350"> <strong>菜单链接地址：</strong><br>
...
sub SaveAdd()
	dim ClassID,ClassName,ShowOnTop,Readme,LinkUrl,PrevOrderID,ShowOnLeft
	dim sql,rs,trs
	...
	ShowOnLeft=trim(request("ShowOnLeft"))
...
sub SaveModify()
	dim ClassName,Readme,ShowOnTop,LinkUrl,ShowOnLeft
	dim trs,rs
	dim ClassID,sql,rsClass,i
	dim SkinCount,LayoutCount
	ClassID=trim(request("ClassID"))
...
sub DownOrder()
	...
	sqlOrder="select * From MenuClass where ParentID=0 and RootID>" & cRootID & " order by RootID"
	set rsOrder=server.CreateObject("adodb.recordset")
	rsOrder.open sqlOrder,conn,1,3
	if rsOrder.bof and rsOrder.eof then
	...
	rsOrder.movenext
	if rsOrder.eof then
		conn.execute("update MenuClass set NextID=0 where ClassID=" & ClassID)
	else
		rsOrder("PrevID")=ClassID
		rsOrder.update
		conn.execute("update MenuClass set NextID=" & rsOrder("ClassID") & " where ClassID=" & ClassID)
...
sub UpOrderN()
	...
	'要移动的栏目信息
	set rs=conn.execute("select ParentID,OrderID,ParentPath,child,PrevID,NextID From MenuClass where ClassID="&ClassID)
	ParentID=rs(0)
	OrderID=rs(1)
	...
	set rs=nothing
	if child>0 then
		set rs=conn.execute("select count(*) From MenuClass where ParentPath like '%"&ParentPath&"%'")
		oldorders=rs(0)
		rs.close
		set rs=nothing
	else
	...
	sql="select ClassID,OrderID,child,ParentPath,PrevID,NextID From MenuClass where ParentID="&ParentID&" and OrderID<"&OrderID&" order by OrderID desc"
	set rs=server.CreateObject("adodb.recordset")
	rs.open sql,conn,1,3
	i=1
	do while not rs.eof
		tOrderID=rs(1)
		conn.execute("update MenuClass set OrderID="&tOrderID+oldorders+i&" where ClassID="&rs(0))
		if rs(2)>0 then
			ii=i+1
			set trs=conn.execute("select ClassID,OrderID From MenuClass where ParentPath like '%"&rs(3)&","&rs(0)&"%' order by OrderID")
			if not (trs.eof and trs.bof) then
				do while not trs.eof
					conn.execute("update MenuClass set OrderID="&tOrderID+oldorders+ii&" where ClassID="&trs(0))
					ii=ii+1
					trs.movenext
				loop
			end if
		...
		if i>MoveNum then
			rs(4)=ClassID
			rs.update
			conn.execute("update MenuClass set NextID=" & rs(0) & " where ClassID=" & ClassID)		
			exit do
		end if
		rs.movenext
	loop
	...
	if rs.eof then
		...
		rs(5)=ClassID
		rs.update
		conn.execute("update MenuClass set PrevID=" & rs(0) & " where ClassID=" & ClassID)
	end if	
	rs.close
	set rs=nothing
	
	'更新所要排序的栏目的序号
	conn.execute("update MenuClass set OrderID="&tOrderID&" where ClassID="&ClassID)
	'如果有下属栏目，则更新其下属栏目排序
	if child>0 then
		i=1
		set rs=conn.execute("select ClassID From MenuClass where ParentPath like '%"&ParentPath&"%' order by OrderID")
		do while not rs.eof
			conn.execute("update MenuClass set OrderID="&tOrderID+i&" where ClassID="&rs(0))
			i=i+1
			rs.movenext
		loop
		rs.close
	...
	call CloseConn()
	response.Redirect "Admin_Class_Menu.asp?Action=OrderN"
end sub
...
sub DownOrderN()
	...
	sql="select ClassID,OrderID,child,ParentPath,PrevID,NextID From MenuClass where ParentID="&ParentID&" and OrderID>"&OrderID&" order by OrderID"
	set rs=server.CreateObject("adodb.recordset")
	rs.open sql,conn,1,3
	i=0      '同级栏目
	ii=0     '同级栏目和子栏目
	do while not rs.eof
		conn.execute("update MenuClass set OrderID="&OrderID+ii&" where ClassID="&rs(0))
		if rs(2)>0 then
			set trs=conn.execute("select ClassID,OrderID From MenuClass where ParentPath like '%"&rs(3)&","&rs(0)&"%' order by OrderID")
			if not (trs.eof and trs.bof) then
				do while not trs.eof
					ii=ii+1
					conn.execute("update MenuClass set OrderID="&OrderID+ii&" where ClassID="&trs(0))
					trs.movenext
				loop
			end if
			trs.close
	...
	'更新所要排序的栏目的序号
	conn.execute("update MenuClass set OrderID="&OrderID+ii&" where ClassID="&ClassID)
	'如果有下属栏目，则更新其下属栏目排序
	if child>0 then
		i=1
		set rs=conn.execute("select ClassID From MenuClass where ParentPath like '%"&ParentPath&"%' order by OrderID")
		do while not rs.eof
			conn.execute("update MenuClass set OrderID="&OrderID+ii+i&" where ClassID="&rs(0))
			i=i+1
			rs.movenext
		loop
		rs.close
...
sub SaveReset()
	...
	sql="select ClassID From MenuClass order by RootID,OrderID"
	set rs=server.CreateObject("adodb.recordset")
	rs.open sql,conn,1,1
	...
	do while not rs.eof
		...
		rs.moveprevious
		conn.execute("update MenuClass set RootID=" & i & ",OrderID=0,ParentID=0,Child=0,ParentPath='0',Depth=0,PrevID=" & PrevID & ",NextID=" & NextID & " where ClassID=" & rs(0))
		PrevID=rs(0)
		i=i+1
		rs.movenext
	loop
	...
	SuccessMsg="复位成功！请返回<a href='Admin_Class_Menu.asp'>栏目管理首页</a>做栏目的归属设置。"
...
sub SaveUnite()
	...
	set rs=conn.execute("select Child from MenuClass where ClassID=" & TargetClassID)
	if rs.bof and rs.eof then
		FoundErr=True
	...
	'得到当前栏目信息
	set rs=conn.execute("select ClassID,ParentID,ParentPath,PrevID,NextID,Depth from MenuClass where ClassID="&ClassID)
	iParentID=rs(1)
	Depth=rs(5)
	if iParentID=0 then
		ParentPath=rs(0)
	...
	'判断是否是合并到其下属栏目中
	set rs=conn.execute("select ClassID from MenuClass where ClassID="&TargetClassID&" and ParentPath like '"&ParentPath&"%'")
	...
	'得到当前栏目的下属栏目ID
	set rs=conn.execute("select ClassID from MenuClass where ParentPath like '"&ParentPath&"%'")
	i=0
	if not (rs.eof and rs.bof) then
		do while not rs.eof
	...
	'删除被合并栏目及其下属栏目
	conn.execute("delete from MenuClass where ClassID in ("&ParentPath&")")
