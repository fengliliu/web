    ...
    <div class="you_nr">
        ...
        <%
        	if classtype="" Then
        		...
        		sql = "select * from Aboutus where title='服务项目'"
        		Content="暂无信息"
        		set rs=server.CreateObject("adodb.recordset")
        	...
        	set rs = server.CreateObject("adodb.recordset")
        	sql = " Select id,title,FirstImageName,SmallContent,adddate From News Where BigClassName='服务项目' "
        	sql = sql&" And smallclassname='"&classtype&"'"
        	if classtype="商务庆典" then
        		sql = " Select id,title,FirstImageName,SmallContent,adddate From News Where BigClassName='商务庆典' "
        	end if
        	...
        	Sub List()
        		dim i
        		i=0
        		if classtype="礼仪庆典" or classtype="商业活动" or classtype="文艺演出" then
        				Do While Not rs.Eof
        %>
              <dl class="you_nr_y">
                <dt><img src="/images/heart.jpg" /></dt>
                <dd><span class="nrbt"><a href="/show<%=rs("id")%>.html"><%=rs("title")%></a></span><span class="date">[<%=FormatDate(rs("adddate"),14)%>]</span></dd>
              </dl>
        <%
        			i=i+1
        ...
        %>
              <dl class="you_nr_ri">
                <dt style="width: 50%;"><a href="/show<%=rs("id")%>.html"><img src="/<%=rs("FirstImageName")%>" /></a></dt>
                <dd style="padding-left: 80px "><a href="/show<%=rs("id")%>.html"><b><%=rs("title")%></b></a></dd>
              </dl>
        <%
        		i=i+1
        	      if i>=MaxPerPage then exit do
